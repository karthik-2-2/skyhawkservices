...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($step == 1) {
        ...
        if (pilotExists($conn, $phone)) {
            $security_question = getPilotSecurityQuestion($conn, $phone);
        ...
        if (verifyPilotSecurityAnswer($conn, $phone, $question_id, $answer)) {
        ...
        if ($new_password !== $confirm_password) {
            ...
            if (updatePilotPassword($conn, $phone, $new_password)) {
...
<head>
    ...
    <title>Forgot Password - Pilot</title>
    <style>
        * {
            margin: 0;
            padding: 0;
